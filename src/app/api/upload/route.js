...
export async function POST(req) {
  ...
  if (!ALLOWED_MIME.includes(file.type || "")) return new Response("Unsupported file type", { status: 415 });
  if (file.size > MAX_BYTES) return new Response("Too large", { status: 413 });

  const inputBuf = Buffer.from(await file.arrayBuffer());

  const orig = file.name || "upload";
  const baseName = path.parse(orig).name.replace(/[^a-z0-9-_]+/gi, "_").toLowerCase();
